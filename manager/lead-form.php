<?php
// manager/lead-form.php — Manager: Add / Edit Lead
...
$statuses  = ['New','Contacted','Follow-up','Interested','Converted','Closed','Rejected'];
    ...
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if (!$errors) {
            ...
            if ($isEdit) {
                ...
                logActivity($_SESSION['user_id'], 'Edit Lead', "Lead #$id updated");
                // Notify assigned employee
                if ($dbData['assigned_to']) {
                    addNotification($dbData['assigned_to'], 'Lead Updated',
                        "Lead '{$fullName}' has been updated.", 'info',
                        APP_URL.'/employee/lead-detail.php?id='.$id);
                }
                ...
                logActivity($_SESSION['user_id'], 'Create Lead', "Lead #$newId created");
                // Notify assigned employee
                if ($dbData['assigned_to']) {
                    addNotification($dbData['assigned_to'], 'New Lead Assigned',
                        "A new lead '{$fullName}' has been assigned to you.", 'success',
                        APP_URL.'/employee/lead-detail.php?id='.$newId);
                }
            ...
            redirect(APP_URL . '/manager/' . $listPage);
        }
    }
}
  ...
  <div class="col-12 col-xl-9">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <a href="<?= APP_URL ?>/manager/<?= $backListPage ?>" class="btn btn-sm btn-outline-secondary">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-4">
              ...
              <select name="assigned_to" class="form-select">
                ...
                <option value="<?= $emp['id'] ?>"
                  <?= ($data['assigned_to']??'')==$emp['id']?'selected':'' ?>>
                  <?= e($emp['name']) ?>
                </option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="col-12 d-flex gap-2 justify-content-end">
              <a href="<?= APP_URL ?>/manager/<?= $backListPage ?>" class="btn btn-outline-secondary">Cancel</a>
