<?php
// admin/manager.php — Employee Management (Mobile Responsive)
...
if (isset($_GET['delete'])) {
    ...
    logActivity($_SESSION['user_id'], 'Delete Manager', "Manager #$did deleted");
    ...
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) { $errors[] = 'CSRF error'; }
        ...
        if (!$errors) {
            if ($pid) {
                ...
                $sql .= " WHERE id=?"; $p[] = $pid;
                DB::query($sql, $p);
                logActivity($_SESSION['user_id'],'Edit Manager',"Manager #$pid updated");
                ...
                logActivity($_SESSION['user_id'],'Add Manager','New Manager added');
                setFlash('success','Manager added.');
            }
            redirect(APP_URL.'/admin/employees.php');
        }
    }
